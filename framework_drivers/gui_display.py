...
class WeatherAndCalendarDisplay:
    def __init__(self, weather_controller, calendar_controller):
        ...
        self.main_frame.grid_rowconfigure(0, weight=2)  # Current weather (larger)
        ...
        self.running = True
        self.weather_update_thread = threading.Thread(target=self.weather_update_loop, daemon=True)
        self.calendar_update_thread = threading.Thread(target=self.calendar_update_loop, daemon=True)
    ...
    def update_weather(self, weather_data_list):
        ...
        self.temp_label.config(text=f"{current_weather.temperature}°{current_weather.temperature_unit}")
        self.condition_label.config(text=current_weather.short_forecast)
        self.humidity_label.config(text=f"Humidity: {current_weather.relative_humidity}%")
        self.wind_label.config(text=f"Wind: {current_weather.wind_speed} {current_weather.wind_direction}")
    ...
    def update_hourly_forecast(self, weather_data_list):
        ...
        for i, weather_data in enumerate(weather_data_list[:3]):  # Limit to 3 periods
    ...
    def weather_update_loop(self):
        ...
        while self.running:
            try:
                weather_data_list = self.weather_controller.get_weather(latitude, longitude, periods_count=3)
    ...
    def calendar_update_loop(self):
        ...
        while self.running:
            try:
                events = self.calendar_controller.get_today_events(max_results=10)
                # Update UI in main thread
